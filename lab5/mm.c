...
static void requestMoreSpace(size_t reqSize) {
  ...
  FPRINTF(stderr,"@requestMoreSpace reqsize is %ld \n",reqSize);
...
void* mm_malloc (size_t size) {
  ...
  BlockInfo * ptrRemain = NULL;
  BlockInfo * ptrFollowing = NULL;
  ...
  if (size <= MIN_BLOCK_SIZE) {
    ...
    reqSize = ALIGNMENT * ((size + ALIGNMENT - 1) / ALIGNMENT);
		 ...
		 FPRINTF(stderr,"USE tag1\n");
  if( (ptrFreeBlock = (BlockInfo *)searchFreeList(reqSize )) == NULL){ 
	     requestMoreSpace (reqSize);
		 FPRINTF(stderr,"USE MORESPACE\n");
		 if((ptrFreeBlock = (BlockInfo *)searchFreeList(reqSize )) == NULL) 
			 FPRINTF(stderr,"!!!!!!!!!!!!!!!!!BUG@BRK\n");
  }
	  ...
	  if(blockSize == reqSize ){
		ptrFollowing = (BlockInfo * ) (POINTER_ADD(ptrFreeBlock,SIZE(ptrFreeBlock->sizeAndTags)));
		precedingBlockUseTag = ptrFreeBlock->sizeAndTags & 7;
		(*(size_t*)(ptrFreeBlock)) = precedingBlockUseTag | TAG_USED;
		((BlockInfo*)(POINTER_ADD(ptrFreeBlock,blockSize)))->sizeAndTags &= TAG_PRECEDING_USED; 
		if(ptrFollowing->sizeAndTags & TAG_USED){
            ptrFollowing->sizeAndTags |= TAG_PRECEDING_USED;      
		 }
		else {
            ptrFollowing->sizeAndTags |= TAG_PRECEDING_USED;      
			*((size_t *)POINTER_ADD(ptrFollowing,SIZE(ptrFollowing->sizeAndTags)-WORD_SIZE))   |= TAG_PRECEDING_USED;
		}
		removeFreeBlock(ptrFreeBlock);
		 FPRINTF(stderr,"==========================================\n");
		return (void *)( POINTER_ADD(ptrFreeBlock,WORD_SIZE)) ;
	  }
	  else if ((blockSize - reqSize) >= MIN_BLOCK_SIZE)
	  {
        precedingBlockUseTag = ptrFreeBlock->sizeAndTags & 7 ;

		*((size_t* )ptrFreeBlock) = reqSize | TAG_USED |(precedingBlockUseTag & TAG_PRECEDING_USED);

	   ptrRemain = (BlockInfo* )(POINTER_ADD(ptrFreeBlock,reqSize));
	   blockSize -=reqSize;
	   ptrRemain->sizeAndTags =( blockSize | TAG_PRECEDING_USED) & ~TAG_USED;
	   blockSize -=WORD_SIZE;
	  ...
	  else 
	  {
		...
		*((size_t* )ptrFreeBlock) = precedingBlockUseTag | TAG_USED |reqSize ;
...
/*  size_t payloadSize;
  ...
  if(ptr == NULL)
   FPRINTF(stderr,"ERROR@mem_free ptr is NULL\n");
...
*/
  ...
  ptr = POINTER_SUB(ptr, WORD_SIZE);
  blockInfo = (BlockInfo *)ptr;
  payloadSize = SIZE(blockInfo->sizeAndTags);

  // if the payloadSize is smaller tan MIN_BLOCK_SIZE, return.
  if (payloadSize < MIN_BLOCK_SIZE)
    return;

  if (((blockInfo->sizeAndTags) & TAG_USED) == 0)
    return;

  if (POINTER_ADD(blockInfo, payloadSize) > mem_heap_hi())
    return;

  // Change the header and footer of this block.
  blockInfo->sizeAndTags = (blockInfo->sizeAndTags) & (~TAG_USED);
  *(size_t *) POINTER_ADD (blockInfo, payloadSize - WORD_SIZE) = blockInfo->sizeAndTags;

  // Change the header and footer of the following block.
